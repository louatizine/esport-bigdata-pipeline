"""
Spark Structured Streaming job for processing player data from Kafka.

This module consumes player events from Kafka, processes them,
...
class PlayerStreamProcessor:
    """
    Processes streaming player data from Kafka and writes to data lake.
    """

    def __init__(self, spark=None):
        """
        Initialize the player stream processor.
    ...
    def process_stream(self, raw_df: DataFrame) -> DataFrame:
        ...
        Returns:
            DataFrame: Processed DataFrame with player data
        """
        logger.info("Processing player stream data")
        ...
        try:
            ...
            # Add data quality transformations and enrichments
            processed_df = processed_df.withColumn(
                "last_match_date",
                to_timestamp(col("last_match_date"))
            ).withColumn(
                "profile_updated_at",
                to_timestamp(col("profile_updated_at"))
            ).withColumn(
                # Calculate win rate if not provided
                "calculated_win_rate",
                when(
                    (col("total_games") > 0) & col("win_rate").isNull(),
                    spark_round(
                        (col("total_wins") / col("total_games")) * 100, 2)
                ).otherwise(col("win_rate"))
            ).withColumn(
                # Calculate KDA if not provided
                "calculated_kda",
                when(
                    col("kda_ratio").isNull() & (col("avg_deaths") > 0),
                    spark_round(
                        (col("avg_kills") + col("avg_assists")) / col("avg_deaths"), 2)
                ).when(
                    col("kda_ratio").isNull() & (col("avg_deaths") == 0),
                    99.99  # Perfect KDA
                ).otherwise(col("kda_ratio"))
            ).withColumn(
                # Player status flag
                "status",
                when(col("active") == True, "active")
                .when(col("active") == False, "inactive")
                .otherwise("unknown")
            ).withColumn(
                # Full player name
                "full_name",
                expr("CONCAT_WS(' ', first_name, last_name)")
            ).withColumn(
                # Games this season (if total_games > 0)
                "has_activity",
                col("total_games") > 0
            )
    ...
    def write_to_parquet(self, processed_df: DataFrame, query_name: str = "player_stream"):
        """
        ...
        """
        output_path = get_output_path("players")
    ...
    def run(self):
        ...
        Returns:
            StreamingQuery: The active streaming query
        """
        logger.info("Starting player streaming pipeline")
        ...
        try:
            ...
            logger.info("Player streaming pipeline running successfully")
            return query

        except Exception as e:
            logger.critical(f"Player streaming pipeline failed: {str(e)}")
            raise


def main():
    """
    Main entry point for player streaming job.
    """
    logger.info("=" * 60)
    logger.info("PLAYER STREAM PROCESSOR - STARTING")
    logger.info("=" * 60)

    try:
        processor = PlayerStreamProcessor()
        query = processor.run()
        ...
        logger.critical(f"Fatal error in player stream processor: {str(e)}")
        sys.exit(1)
    finally:
        logger.info("Player stream processor terminated")
